...
3. [If you have remote content] Deploy Remote artifacts to your Remote Load Path
...
<table>
  ...
  <tr>
    <td>System Files</td>
    <td>catalog.json (content catalog)
settings.json (profile variables, etc)
link.xml (preserves assembly contents for scripts attached to bundles)</td>
...
Note: these files are only used for content update builds</td>
  ...
  <tr>
    <td>Asset Bundles</td>
    <td>A special AssetGroup called Built In Data
Any AssetGroups with Build and Load Paths set to local paths, typically LocalBuildPath and LocalLoadPath</td>
    <td>Any AssetGroups with Build and Load Paths set to remote paths, typically RemoteBuildPath and RemoteLoadPath</td>
  </tr>
</table>
